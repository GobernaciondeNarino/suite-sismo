...
- **Dependencias sin terceros**: D3plus, Leaflet y three.js se sirven desde `assets/vendor/`, dentro del propio plugin. Antes se cargaban por CDN con huella SRI; en la práctica cualquier bloqueador de anuncios, extensión de privacidad o proxy que se interpusiera convertía esa verificación en un bloqueo total y la página se quedaba sin gráficos. Servirlas desde el sitio elimina esa superficie y, de paso, evita que el navegador de quien consulta la página haga peticiones a servidores ajenos. El único recurso externo que el plugin puede pedir es la textura fotográfica del globo, desactivada por defecto y activable con `textura="si"`.
...
6. **Actualizaciones**: al subir de versión una librería hay que sustituir el archivo en `assets/vendor/` y actualizar su constante de versión en `SIS_Shortcodes` — de ella depende el parámetro `ver` que invalida la caché del navegador. El procedimiento está en `assets/vendor/LEEME.md`.
