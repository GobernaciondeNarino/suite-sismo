...
final class SIS_Shortcodes {
	...
	public function registrar_shortcodes() {
		...
		add_shortcode( 'sismos_prediccion_dato', array( $this, 'sc_prediccion_dato' ) );
		...
		add_shortcode( 'sismos_pronostico', array( $this, 'sc_pronostico' ) );
	...
	public function registrar_assets() {
		...
		wp_register_script( 'sis-pronostico', SIS_URL . 'assets/js/pronostico.js', array( 'sis-core' ), SIS_VERSION, true );
	...
	public function sc_analisis( $atts ) {
		...
		$modo = in_array( $atts['modo'], array( 'ambos', 'descriptivo', 'cuantitativo', 'descripcion', 'como_funciona', 'prediccion' ), true ) ? $atts['modo'] : 'ambos';
	...
	/**
	 * [sismos_prediccion_dato] — párrafo predictivo de la vista.
	 *
	 * @param array $atts Atributos.
	 * @return string
	 */
	public function sc_prediccion_dato( $atts ) {
		$atts = $this->fusionar( $this->defaults_bloque(), $atts, 'sismos_prediccion_dato' );
	...
	/**
	 * [sismos_pronostico] — ficha del pronóstico a 6 meses.
	 *
	 * @param array $atts Atributos.
	 * @return string
	 */
	public function sc_pronostico( $atts ) {
		$atts = $this->fusionar( array_merge(
			$this->defaults_consulta(),
			array( 'modo' => 'completo', 'grafico' => 'si' )
		), $atts, 'sismos_pronostico' );

		wp_enqueue_style( 'sis-estilos' );
		wp_enqueue_style( 'sis-grafico-css' );
		wp_enqueue_script( 'sis-pronostico' );

		$modo = in_array( $atts['modo'], array( 'completo', 'resumen', 'umbrales', 'metodo' ), true ) ? $atts['modo'] : 'completo';
		$id   = $this->id();

		ob_start();
		?>
		<div id="<?php echo esc_attr( $id ); ?>" class="sis sis-pronostico"
			style="<?php echo esc_attr( SIS_Estilos::estilo_inline( $atts ) ); ?>"
			data-sis-pronostico
			data-modo="<?php echo esc_attr( $modo ); ?>"
			data-grafico="<?php echo esc_attr( 'no' === $atts['grafico'] ? '0' : '1' ); ?>"
			<?php echo $this->data_consulta( $atts ); // phpcs:ignore WordPress.Security.EscapeOutput ?>>
			<?php echo $this->skeleton( __( 'Calculando el pronóstico a 6 meses…', 'sismos-narino' ) ); // phpcs:ignore WordPress.Security.EscapeOutput ?>
			<?php echo $this->pie_fuentes( 'Estimación propia sobre el catálogo del USGS · No es un aviso oficial (SGC/UNGRD)' ); // phpcs:ignore WordPress.Security.EscapeOutput ?>
		</div>
		<?php
		return ob_get_clean();
	}
	...
	public function sc_datos( $atts ) {
		...
		$recurso = in_array( $atts['recurso'], array( 'eventos', 'estadistica', 'pronostico' ), true ) ? $atts['recurso'] : 'eventos';
		$id      = $this->id();
