...
return array(
	...
	'pronostico_mensual'    => array(
		'descripcion'   => 'Sismos observados mes a mes y sismos esperados para los próximos seis meses, dibujados como una sola línea continua. La parte observada procede del catálogo; la pronosticada, del modelo estadístico del plugin, que combina el fondo de largo plazo, el estado reciente de la actividad y las réplicas pendientes de cualquier secuencia activa. El pronóstico se recalcula con cada actualización del catálogo, de modo que las cifras cambian a medida que llegan datos nuevos.',
		'analisis'      => 'Lo que se pronostica es la TASA de actividad, no un sismo concreto. La curva esperada casi nunca es plana: arranca cerca del ritmo reciente —si los últimos meses fueron tranquilos, empieza baja— y va revirtiendo hacia el promedio histórico a medida que avanza el horizonte, porque a seis meses vista el estado reciente ya no aporta información. Si acaba de ocurrir un sismo importante, los primeros meses aparecen claramente elevados por las réplicas esperadas y luego descienden siguiendo la ley de Omori. Las diferencias entre un mes y el siguiente son pequeñas: la señal está en el nivel general, no en el detalle.',
		'como_funciona' => 'La tasa mensual esperada suma tres componentes: el fondo climatológico de la ley de Gutenberg-Richter ajustada sobre toda la ventana; el estado reciente proyectado con suavizado exponencial de tendencia amortiguada (Holt amortiguado), cuyo peso decae con el horizonte; y las réplicas de un eventual sismo detonante del último año, según la ley de Omori-Utsu con productividad de Reasenberg y Jones.',
	),

	'pronostico_banda'      => array(
		'descripcion'   => 'Valor esperado y banda de incertidumbre del pronóstico para cada uno de los próximos seis meses. La banda es el intervalo de predicción al 90 %: dentro de él debería caer el número real de sismos en nueve de cada diez ocasiones si el modelo es correcto.',
		'analisis'      => 'La anchura de la banda es la parte más honesta del pronóstico. Cuando la tasa esperada es baja —menos de un sismo al mes— el intervalo casi siempre incluye el cero: es perfectamente posible que no ocurra ninguno, y también que ocurran varios. Esa amplitud no es un defecto del modelo, es la naturaleza aleatoria del proceso: los sismos llegan agrupados en el tiempo. Cualquier lectura que se fije solo en el valor central y olvide la banda estará sobreinterpretando el resultado.',
		'como_funciona' => 'La banda se construye con el intervalo de predicción exacto de una distribución de Poisson de media igual a la tasa esperada, usando su relación con la distribución chi-cuadrado. No incluye la incertidumbre del propio ajuste de parámetros, así que es un límite inferior de la incertidumbre total.',
	),

	'pronostico_umbrales'   => array(
		'descripcion'   => 'Probabilidad de que ocurra al menos un sismo de cada magnitud o mayor durante los próximos seis meses en el ámbito seleccionado. Se calcula repartiendo la tasa esperada entre magnitudes con la ley de Gutenberg-Richter y aplicando después la probabilidad de Poisson.',
		'analisis'      => 'Estas cifras describen la amenaza, no anuncian un evento. Que la probabilidad de un sismo moderado sea alta es lo normal en una zona de subducción activa: significa que ese tamaño forma parte del comportamiento habitual de la región. Las probabilidades bajas de los sismos grandes no son tranquilizadoras por sí solas: un 3 % en seis meses equivale a algo que ocurre cada pocas décadas, y las consecuencias de ese escenario son las que justifican preparar la respuesta. La utilidad práctica está en comparar umbrales entre sí y seguir su evolución entre actualizaciones.',
		'como_funciona' => 'A partir del número esperado de sismos por encima de la magnitud de completitud, la proporción que supera cada umbral se obtiene con una ley de Gutenberg-Richter truncada en la mayor ruptura creíble del dominio. La probabilidad de al menos un evento es 1 − e^(−N), con N el número esperado sobre ese umbral.',
	),

	'periodo_retorno'       => array(
		'descripcion'   => 'Cada cuántos años, en promedio, cabe esperar un sismo de cada magnitud o mayor en el ámbito seleccionado. Es el inverso de la tasa anual de la ley de Gutenberg-Richter y una de las cifras más usadas en normativa de construcción y en planeación de emergencias.',
		'analisis'      => 'El periodo de retorno es un promedio de largo plazo, no un calendario: que un sismo tenga periodo de retorno de veinte años no significa que ocurra puntualmente cada veinte, ni que después de uno se abra un plazo de seguridad. Los sismos no tienen memoria en este modelo, así que la probabilidad del año que viene es la misma haya ocurrido o no recientemente. La cifra sirve para dimensionar la amenaza durante la vida útil de una obra: un edificio de cincuenta años atravesará, en promedio, más de dos periodos de retorno de veinte.',
		'como_funciona' => 'Se calcula como el inverso de la tasa anual derivada del pronóstico para cada umbral de magnitud, extrapolando a un año la tasa esperada en la ventana de seis meses. Los umbrales por debajo de la magnitud de completitud no se publican, porque el catálogo no los registra de forma fiable.',
	),
);
